...
- 배열
    - for문
	- 배열 이름은 주소다.
	- 동일한 자료형이라서 찾기 쉬운것이 장점이다.(index)
	- 단점은 할당과 해제를 수동으로 처리해야 함..
	- 배열 크기는 바꿀 수 없음..
...
## 13일차
- 예외처리
   - try{if(!n2) throw n2; 예외처리 할 코드} -- catch(int exception){}

- C++ 정리
   - vector : 자동으로 메모리가 할당  -> 배열의 단점을 보완할 수 있기 때문에 자주 사용
       - 템플릿(template)을 통해 vector을 구현해야 함
	   - 첫번째 방은 v.front(), 마지막 방은 v.back()라고 한다.
